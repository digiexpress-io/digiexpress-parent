...
const ${updateName} = ${variableName}.update({
  ...
  visitHookByPath(moduleName: string) {
    return `
...
${this._hookTypeEntries.join('\r\n')}
  ...
  close(moduleName: string): TreeFile[] {
    const result = [
      "import { Hook as RootHook } from './fetch/__root'",
      ...this._importEntries,'','',  
      ...this._hookEntries,'','',
      this.visitHookByPath(moduleName), 
...
export function parseTree(moduleName: string, files: SourceFile[]): TreeFile[] {
  const visitor = new TreeVisitor();
  files.forEach(src => {
    visitor.visit(src);
  })
  return visitor.close(moduleName);
}
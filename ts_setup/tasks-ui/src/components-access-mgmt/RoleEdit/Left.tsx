...
import { useRoleEdit } from './RoleEditContext';
import { useAm } from 'descriptor-access-mgmt';


//TODO: Status field
const RoleName: React.FC<{}> = () => {
  const { setName, entity } = useRoleEdit();
  const intl = useIntl();
  ...
  return (<TextField InputProps={{ disableUnderline: true }} variant='standard'
    placeholder={intl.formatMessage({ id: 'permissions.roles.name.edit.placeholder' })}
    fullWidth
    value={entity.name}
...
const RoleDescription: React.FC<{}> = () => {
  const { setDescription, entity } = useRoleEdit();
  const intl = useIntl();
  ...
  return (<TextField InputProps={{ disableUnderline: true }} variant='standard'
    placeholder={intl.formatMessage({ id: 'permissions.roles.description.edit.placeholder' })}
    fullWidth
    multiline
    minRows={3}
    maxRows={6}
...
const RoleCommitComment: React.FC<{}> = () => {
  const { setCommitComment, entity } = useRoleEdit();
  const intl = useIntl();

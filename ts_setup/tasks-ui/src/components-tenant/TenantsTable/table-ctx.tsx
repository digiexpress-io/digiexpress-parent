...
type TenantPagination = Pagination.TablePagination<TenantEntryDescriptor>;

interface TableConfigProps {
  loading: boolean;
  group: Group
  content: TenantPagination,
  setContent: React.Dispatch<React.SetStateAction<TenantPagination>>,
}
...
interface TableProps {
  ...
  data: {
    group: Group,
    defaultOrderBy: keyof TenantEntryDescriptor,
    loading: boolean;
  }
}
...
function CustomTable(props: TableProps) {
  ...
  const [content, setContent] = React.useState<TenantPagination>(new Pagination.TablePaginationImpl<TenantEntryDescriptor>({
    src: records ?? [],
  ...
  React.useEffect(() => {
    setContent((c: TenantPagination) => c.withSrc(records ?? []));
  }, [records, setContent]);
        ...
        <Box alignSelf="center" flexGrow={1}></Box> {
          loading ? null :
            (<TablePagination
              ...
              page={content.page}
              onPageChange={(_event, newPage) => setContent((state: TenantPagination) => state.withPage(newPage))}
              onRowsPerPageChange={(event: React.ChangeEvent<HTMLInputElement>) => setContent((state: TenantPagination) => state.withRowsPerPage(parseInt(event.target.value, 10)))}
            />)
        }
      </Box>
    </Box>

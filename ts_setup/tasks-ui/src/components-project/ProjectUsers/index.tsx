...
import { SearchFieldPopover } from 'components-task/SearchField';
import { useMockPopover } from 'components-task/TaskTable/MockPopover';
import Client from 'client';
import { AvatarCode } from 'descriptor-project';
...
const AvatarsOnly: React.FC<{
  ...
  return task.users.length ?
    (<AvatarGroup spacing='medium'>
      {(task.userAvatars ?? Client.resolveAvatar(task.users)).map((assignee: AvatarCode) => (<UserAvatar key={assignee.value}>{assignee}</UserAvatar>))}
    </AvatarGroup>) :
    (<UserAvatar />)
}
...
const FullnamesAndAvatars: React.FC<{
  ...
  return task.users.length ?
    (<Stack spacing={1}>
      {(task.userAvatars ?? Client.resolveAvatar(task.users)).map((assignee: AvatarCode) => (<Box key={assignee.value} display='flex' alignItems='center' sx={{ cursor: 'pointer' }}>
...
const TaskAssignees: React.FC<{
  ...
  const [newAssignees, setNewAssignees] = React.useState(task.users);
  const { setSearchString, searchResults } = Context.useProjectUsers({ assignees: newAssignees });
  ...
  function onSubmit() {
    const isChanges = newAssignees.sort().toString() !== task.users.sort().toString();
    if (isChanges) {

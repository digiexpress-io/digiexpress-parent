...
export const WithFormProvider: React.FC<FormProviderProps & { id: string; disabled?: boolean }> = (props) => {
  ...
  const { id, executionId, variant, onAfterComplete, onCancel, disabled } = props;
  const store = useFormStore({ id });
  const contextValue = React.useMemo(() => Object.freeze({ store, variant, executionId, disabled: disabled === true, onAfterComplete, onCancel }), 
    [store, variant, executionId, disabled, onAfterComplete, onCancel])

...
export class HintUtils {
  static get(keyword: Fs.YamlFlowKeyword, node: any): any {
    const result = node.children ? node.children[keyword] : node[keyword];
    return result;
  }

  static isNonNull(name: Fs.YamlFlowKeyword, node: any): boolean {
  ...
  static isAfterChildrenOf(container: Container, node: Fs.Yaml): boolean {
  ...
  static taskHasBody(node: Fs.Yaml): boolean {
  ...
  static getStringFlowInputs(flow: Fs.FlowAst): Fs.YamlFlowInputNode[] {
  ...
  static getFormReturnsCode(container: Container, returnsNode: Fs.Yaml | undefined): string {
    if (!returnsNode) {
      return '';
    }
  ...
  static getFormOutputFields(container: Container, task: Fs.YamlFlowTaskNode): { name: string, type: string }[] {
    const form = HintUtils.get('form', task);
    if (!form) {
      return [];
    }
  ...
  static isEndOfLine(container: Container, node: Fs.Yaml): boolean {
    ...
    return container.nav.currentColumn >= last.source.line.length;
  }

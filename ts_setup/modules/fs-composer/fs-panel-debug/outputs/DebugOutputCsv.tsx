...
const mapCsvRows = (debug: string): Fs.CsvRow[] => {
    if (!debug) {
        return [];
    }
    const result: Fs.CsvRow[] = [];
    ...
    for (let i = 0; i < inputLines.length; i++) {
        ...
        const csvRow: Fs.CsvRow = { id: rowId, inputs: [], outputs: [] };
...
const DebugOutputCsv: React.FC<{ debug: string }> = ({ debug }) => {
    ...
    return (<>
      {csvRows.map((csvRow: Fs.CsvRow) => <DebugOutputCsvRow key={csvRow.id} csvRow={csvRow} index={csvRow.id}/>)}
    </>);
}

...
export class VersionInfoBuilder {
  ...
  render(): void {
    ...
    const logoContent = LOGOS[this.selectedLogo];
    ...
    // Render logo
    const logoResult = logoCommand.execute({ logo: logoContent, theme: this.theme });
    ...
    // Render separator
    const separatorResult = separatorCommand.execute({ length: separatorLength, theme: this.theme });

...
const LocaleLabels: React.FC<LocaleLabelsProps> = (props) => {
  ...
  const handleRemoveLabel = (id: TagomiApi.LocaleId) => {
    const newSelection: Record<string, SelectedValue> = {};
      ...
      <Box>
        ...
        {rows.map((row, index) => (
          <Grid2 container alignItems='center' p={theme.spacing(1)} key={index}>
            <Grid2 size={{ md: 3, lg: 3, xl: 3 }} pl={theme.spacing(2)}><Typography fontWeight='bold'>{site.locales[row.locale]?.localeCode}</Typography></Grid2>

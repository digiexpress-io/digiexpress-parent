...
export function useSavedTableFilters(tableId: string) {
  ...
  async function onSave(
    ...
    let toBeSaved: TableConfig | undefined;
    let dataId: string;
    ...
    const configs = (data ?? []).reduce<Record<string, TableConfig>>((collector, next) => {
    ...
    const _updated = await save(settingId, Object.values(configs));

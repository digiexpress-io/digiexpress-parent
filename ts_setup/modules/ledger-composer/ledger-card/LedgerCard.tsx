...
import { LedgerCardMenu } from './LedgerCardMenu';

import { flashyCardColorsById, CardStyleDefinition, useCardThemeConfig } from './cardThemeConfig';
import { CardId, CardStyleKey } from './CardConfigContext';
...
export const LedgerCard: React.FC<LedgerCardProps> = (props) => {
  ...
  return (<>
    {props.editDialog}
    {props.isCardHidden ? (<></>) : (
      <LedgerSectionCard className={classes.dataCard} ownerState={props} id={props.id}>
        <Box className={classes.title}>
          ...
          {props.showEditButton && <Button variant='text' onClick={handleEdit}>{intl.formatMessage({ id: 'LedgerCard.title.edit', defaultMessage: 'Edit' })}</Button>}
          ...
          <LedgerCardMenu cardId={props.id}
            anchorEl={anchorEl}
            open={menuOpen}
...
const MUI_NAME = 'LedgerSectionCard';
const LedgerSectionCard = styled(Box, {
  name: MUI_NAME,
  slot: 'dataCard',
...
})<{ ownerState: LedgerCardProps }>(({ theme, ownerState }) => {
  ...
  const baseStyles: SxProps = {
    ...
    '& .LedgerSectionCard-cardBody': {
      backgroundColor: theme.palette.background.default,
      border: `1px solid ${theme.palette.divider}`,
      borderTop: 'none',
    },
    '& .LedgerSectionCard-titleNotifier': {

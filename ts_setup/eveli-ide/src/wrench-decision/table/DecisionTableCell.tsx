...
const DecisionTableCell: React.FC<{
...
}> = ({ header, cell, onClick, dt, onChange, errors, row }) => {
  ...
  if (header.valueType === "INTL") {
    ...
    const error = errors.find(e => e.exception?.row === row.order && e.exception?.column === header.order);


    return (
      <TableCell key={cell.header} sx={{ 
        ...
        <ButtonGroup variant='text'>
          {(header.valueSet ?? []).map(locale => (
            <Tooltip title={error?.msg ?? new IntlBuilder({ value: cell.value ?? '{}', header }).getLocaleValue(locale)} key={header.id}>

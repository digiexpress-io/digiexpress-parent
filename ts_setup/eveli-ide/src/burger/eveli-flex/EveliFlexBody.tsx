...
const useUtilityClasses = (ownerState: EveliFlexBodyProps) => {
  const slots = { root: ['root'], cancel: ['cancel'], header: ['header'] };
  const getUtilityClass = (slot: string) => generateUtilityClass('EveliFlexBody', slot);
  return composeClasses(slots, getUtilityClass, {});
}


interface EveliFlexBodyProps {
  children: React.ReactNode;
}



export const EveliFlexBody: React.FC<EveliFlexBodyProps> = (initProps) => {
  ...
  return (
    <EveliFlexBodyRoot ownerState={ownerState} className={classes.root}>
      {initProps.children}
    </EveliFlexBodyRoot>
  )
}
...
const EveliFlexBodyRoot = styled("div", {
  name: 'EveliFlexBody',
  slot: 'Root',
...
})<{ ownerState: EveliFlexBodyProps }>(({ theme }) => {
  return {

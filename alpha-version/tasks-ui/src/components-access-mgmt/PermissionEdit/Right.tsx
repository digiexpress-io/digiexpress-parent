...
import { usePermissionEdit, useTabs } from './PermissionEditContext'; //useTabs
...
const PermissionRoles: React.FC = () => {
  const { roles } = useAm();
  const { addRole, removeRole, entity } = usePermissionEdit();
...
const PermissionPrincipals: React.FC = () => {
  ...
  const { addPrincipal, removePrincipal, entity } = usePermissionEdit();

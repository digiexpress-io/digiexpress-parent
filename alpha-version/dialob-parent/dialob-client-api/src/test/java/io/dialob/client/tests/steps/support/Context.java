    ...
    try {
      while (whenMessageIterator.hasNext()) {
        ...
        if (expectations.isSuccess()) {
          log.info("Step success: \"{}\"", whenMessage.getName());
        }


        if (!expectations.isCompleted()) {
          log.info("Message not completed, stored event: \"{}\"", whenMessage.getName());

          Thread.sleep(100);

          for(final var event : this.eventActions) {
            log.info("Event published: \"{}\"", DialobClientTestImpl.Builder.MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(event));
        ...
        if (!expectations.isSuccess()) {
          log.error("Failed expectations {}", expectations);
  ...
  public void publish(Event event) {
    try {
      if(event instanceof FormUpdatedEvent) {
        log.info("New form published: \"{}\"", DialobClientTestImpl.Builder.MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(event));
        this.executor = null;

package io.resys.thena.git.test;
...
public class SearchBlobFromHistoryDBTest extends DbTestTemplate {
  ...
  public void addSamVimesChanges(int changes, String id) {
    ...
    for(int index = 0; index < changes; index++) { 
      client.git(getRepo().getName()).commit().commitBuilder()
      .branchName("main")
  ...
  public void findByExactMatchFromAllPossibleCases() throws JSONException {
    ...
    final var history = getClient().git(getRepo().getName()).history().blobQuery()
      .branchName("main")
  ...
  public void findByExactMatchAllCommits() {
    ...
    var history = getClient().git(getRepo().getName()).history().blobQuery()
      .branchName("main")
      .matchBy(ImmutableMatchCriteria.builder().type(MatchCriteriaType.LIKE).key("name").value("sam").build())
      .latestOnly(false)
      .get()
    ...
    history = getClient().git(getRepo().getName()).history().blobQuery()
        .branchName("main")
        .matchBy(ImmutableMatchCriteria.builder().type(MatchCriteriaType.EQUALS).key("name").value("sam").build())
        .latestOnly(false)
        .get()

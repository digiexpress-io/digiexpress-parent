...
import io.resys.thena.docdb.spi.git.commits.CommitBatchBuilder.CommitTreeMutator;
...
public class DocCommitBatchBuilderImpl implements DocCommitBatchBuilder {
  ...
  private final DocCommitTreeState commitTree; 
  ...
  public DocBatch build() {
    final var mutator = new DocCommitTreeMutator();
    ...
    final var commit = ImmutableCommit.builder()
      ...
      .build();
    final var ref = ImmutableDocBranch.builder()
        .commit(commit.getId())
  ...
  private static void visitTree(DocCommitTreeState from, DocCommitTreeMutator mutator) {
    if(from.getTree().isPresent()) {
      mutator.getNextTree().putAll(from.getTree().get().getValues());
    }
  }
  
  private static BatchStatus visitEmpty(DocCommitTreeMutator mutator) {
  ...
  private static void visitAppend(
      ...
      DocCommitTreeMutator mutator,
      DocCommitTreeState commitTree) {
    
    final var logger = mutator.getLogger();
    final var nextTree = mutator.getNextTree();
    final var nextBlobs = mutator.getNextBlobs();
    final List<RedundentHashedBlob> hashed = newBlobs.entrySet().stream()
      .map(DocCommitBatchBuilderImpl::visitAppendEntry)
      .collect(Collectors.toList());
    
    
    mergeBlobs.entrySet().stream()
      .map(entry -> DocCommitBatchBuilderImpl.visitMergeEntry(entry, commitTree))
      .forEach(hashed::add);

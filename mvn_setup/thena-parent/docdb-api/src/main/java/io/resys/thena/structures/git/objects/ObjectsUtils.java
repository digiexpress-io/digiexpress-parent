...
public class ObjectsUtils {
  public static Uni<String> getTagCommit(String tagName, GitState ctx) {
    return ctx.query().tags().name(tagName).getFirst()
        .onItem().transform(tag -> tag == null ? null : tag.getCommit());
  }
  public static Uni<String> getRefCommit(String refName, GitState ctx) {
    return ctx.query().refs().name(refName)
        .onItem().transform(ref -> ref == null ? null : ref.getCommit());
  }
  public static Uni<Tree> getTree(Commit commit, GitState ctx) {
    return ctx.query().trees().getById(commit.getTree());
  }
  public static Uni<Commit> getCommit(String commit, GitState ctx) {
    return ctx.query().commits().getById(commit);
  }
  public static Uni<Commit> findCommit(GitState ctx, String anyId) {
    
    return ObjectsUtils.getTagCommit(anyId, ctx)
      .onItem().transformToUni(tag -> {
        if(tag == null) {
          log.info("Can't find from repo: '{}' a tag: '{}' trying to find by ref: '{}'", ctx.getDataSource().getTenant().getName(), anyId, anyId);
      ...
      .onItem().transformToUni(commitId -> {
        if(commitId == null) {
          log.info("Can't find from repo: '{}' a ref: '{}' trying to find by commit: '{}'", ctx.getDataSource().getTenant().getName(), anyId, anyId);
  ...
  public static Uni<Map<String, Blob>> getBlobs(Tree tree, List<MatchCriteria> blobCriteria, GitState ctx) {

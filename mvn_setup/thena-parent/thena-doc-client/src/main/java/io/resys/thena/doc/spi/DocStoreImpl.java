...
import io.resys.thena.doc.api.DocStore;
import io.resys.thena.doc.api.ImmutableThenaDocConfig;
import io.resys.thena.doc.api.ThenaDocConfig;
import io.resys.thena.doc.spi.support.DocStoreException;
import io.resys.thena.doc.spi.support.ImmutableDocumentExceptionMsg;
...
public class DocStoreImpl<T extends DocStore<T>> implements DocStore<T> {
  ...
  protected Uni<T> deleteRepo(String repoName, String headName) {
    ...
    return existingRepo.onItem().transformToUni((repoResult) -> {
      final var repoId = repoResult.getId();
  ...
  public static class Builder<T extends DocStore<T>> {
    ...
    public T build() {
      ...
      if(pgPool == null) {
        ...
        this.pgPool = io.vertx.mutiny.pgclient.PgPool.pool(connectOptions, poolOptions);
      ...
      final ThenaDocConfig config = ImmutableThenaDocConfig.builder()
          .client(new DocClientImpl(new DocDataSourceImpl(dataSource)))
          .repoId(repoName)

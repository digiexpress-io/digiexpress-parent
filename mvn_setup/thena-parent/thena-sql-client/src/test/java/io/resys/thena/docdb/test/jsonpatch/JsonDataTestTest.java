...
public class JsonDataTestTest {
  ...
  public void RFC6901_sampleTest() throws IOException {
    final var testData = new JsonObject(toString(JsonDataTestTest.class, "jsonpatch/RFC6901_sample.json"));
  ...
  public void randomData_createTest() throws IOException {
    final var sample = new JsonObject(toString(JsonDataTestTest.class, "jsonpatch/randomData_create.json"));
    final var emptyJsonObject = new JsonObject("{}");
    final var diff = JsonPatch.diff(emptyJsonObject, sample);
  ...
  public void modifiedTest() throws IOException {
    final var sample_start = new JsonObject(toString(JsonDataTestTest.class, "jsonpatch/randomData_create.json"));
    final var sample_end = new JsonObject(toString(JsonDataTestTest.class, "jsonpatch/randomData_modified_1.json"));
    final var diff = JsonPatch.diff(sample_start, sample_end);
    log.debug("Created patch: \r\n{}", diff.getValue().encodePrettily());

    
    Assertions.assertEquals(
        new JsonArray(toString(JsonDataTestTest.class, "jsonpatch/randomData_patch.json")).encodePrettily(), 
        diff.getValue().encodePrettily());

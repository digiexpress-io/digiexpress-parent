...
public class RepoAssert {
  ...
  public static String isName(String value, Supplier<String> message) {
    RepoAssert.isTrue(value.matches(NAME_PATTERN), () -> message.get() + " => Valid name pattern: '" + NAME_PATTERN + "'!");
    return value;
  }
  public static String notEmptyIfDefined(Optional<String> src, Supplier<String> message) {
    ...
    if (object == null || object.isBlank()) {
      throw new RepoAssertException(getMessage(message));
    }
    return object;
  }
  ...
  public static void isTrue(boolean expression, Supplier<String> message, Object ...args) {
    if (!expression) {
      throw new RepoAssertException(getMessage(message, args));
    }
  }
  ...
  public static class RepoAssertException extends RuntimeException {

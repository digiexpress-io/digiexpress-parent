...
public class InternalFsDirentSequenceSql implements FsQueries.InternalDirentSequence {
  ...
  public Uni<Long> nextVal() {
    final var sql = registry.dirents().getNextRefSequence();
    ...
    return dataSource.getClient().preparedQuery(sql.getValue())
        ...
        .onFailure().invoke(e -> errorHandler.deadEnd(sql.failed(e, "Can't find next dirent ref sequence!")));
  }
  @Override
  public Uni<List<Long>> nextVal(long howMany) {
    final var sql = registry.dirents().getNextRefSequence(howMany);
    if(log.isDebugEnabled()) {
      log.debug("User nextVal query, with props: {} \r\n{}", 
    ...
    return dataSource.getClient().preparedQuery(sql.getValue())
        ...
        .onFailure().invoke(e -> errorHandler.deadEnd(sql.failed(e, "Can't find next dirent ref sequence: %s!", howMany)));
  }
}

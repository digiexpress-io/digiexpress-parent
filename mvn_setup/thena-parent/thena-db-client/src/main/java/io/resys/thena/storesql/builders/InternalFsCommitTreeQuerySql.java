...
public class InternalFsCommitTreeQuerySql implements FsQueries.InternalCommitTreeQuery {
  ...
  public InternalFsCommitTreeQuerySql(ThenaSqlDataSource dataSource) {
    super();
  ...
  @Override
  public Uni<List<FsCommitTree>> findAllByDirentId(String direntId) {
    
    final var sql = registry.commitTrees().findAllByDirentId(direntId);
    ...
    return dataSource.getClient().preparedQuery(sql.getValue())
        ...
        .collect().asList()
        .onFailure().invoke(e -> errorHandler.deadEnd(sql.failed(e, "Can't find '%s'!", FsDocType.FS_COMMIT_TREE)));
  }
}

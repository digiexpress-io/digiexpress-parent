...
public class FsPrinter {
  ...
  public FsPrinter(DbState state) {
    super();
    this.state = state;
  } 
  ...
  public String internalPrinting(Tenant repo, boolean isStatic, final Map<String, String> collector) {
    ...
    final var ctx = state.toFsState(repo);
    ...
    .transform(items -> {
      ...
      for(final var item : items.stream()
          .sorted((a, b) -> ComparisonChain.start()
              .compare(
                  ID.apply(a.getDirents().values().iterator().next().getId()), 
                  ID.apply(b.getDirents().values().iterator().next().getId())
              ).result())
          .toList()) {
        
        
        final var mission = item.getDirents().values().iterator().next();      
        result.append("Dirent: ").append(ID.apply(mission.getId())).append(System.lineSeparator());

        ID.apply(mission.getDirentRef());
        ...
        for(final var data : item.getDirentLabels().values().stream()
            .sorted((a, b) -> ComparisonChain.start()
                .compare(ID.apply(a.getCommitId()), ID.apply(b.getCommitId()))

...
import io.resys.thena.ledger.client.api.LedgerQueryActions.ReferenceNumberQuery;
...
public class ReferenceNumberQueryImpl implements ReferenceNumberQuery {
  private final Uni<BbDb> startingState;
  ...
  public Multi<String> findNext(int howMany) {
    if(howMany == 1) {
      return startingState
          .onItem().transformToUni(state -> state.query().queryLedgerSeq().getNext())
          .onItem().transformToMulti(nextVal -> Multi.createFrom().item(formatSequence(nextVal)));
    }
    
    return startingState
        .onItem().transformToMulti(state -> state.query().queryLedgerSeq().findNext(howMany))

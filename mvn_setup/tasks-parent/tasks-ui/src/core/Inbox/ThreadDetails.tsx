...
const CollapsibleMessage: React.FC<{ message: Message }> = ({ message }) => {
  ...
  return (
    <Box sx={{ borderTop: '1px solid', borderBottom: '1px solid', borderColor: 'divider' }}>
      ...
      <PaddedTypography noWrap>{text}</PaddedTypography>
    </Box>
  )
}
...
const AttachmentListItem: React.FC<{ attachment: Attachment }> = ({ attachment }) => {
  return (
    <ListItem sx={{ borderTop: '1px solid', borderBottom: '1px solid', borderColor: 'divider' }}>
    ...
    <>
      <Box>
        {messages
          ...
          .map(message => <CollapsibleMessage key={message.id} message={message} />)
        }
      </Box>
...
const ThreadDetails: React.FC<{ thread: Thread }> = ({ thread }) => {
  const { id, userName, topicName, formName } = thread;

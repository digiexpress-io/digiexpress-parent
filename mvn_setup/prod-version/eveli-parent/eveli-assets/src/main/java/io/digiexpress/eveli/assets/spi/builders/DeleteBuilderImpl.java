...
public class DeleteBuilderImpl implements EveliAssetComposer.DeleteBuilder {
  private final EveliAssetClient eveliAssetClient;


  @Override
  public Uni<Entity<Publication>> publication(String publicationId) {
    final Uni<AssetState> query = eveliAssetClient.queryBuilder().head();
    
    return query.onItem().transformToUni(state -> {
      final var currentState = state.getPublications().get(publicationId);

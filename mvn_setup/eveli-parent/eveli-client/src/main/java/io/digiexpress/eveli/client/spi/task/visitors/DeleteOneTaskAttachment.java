...
public class DeleteOneTaskAttachment implements TaskStoreConfig.MergeTaskVisitor<TaskClient.Task> {
  private final String userId;
  private final String taskId;
  private final String name;
  
  private void deleteTaskAttachment(String fileName, MergeMission merge) {
    var newLinks = merge.getCurrentState().getLinks().values().stream()
        .filter(l -> TaskMapper.LINK_TYPE_ATTACHMENT.equals(l.getLinkType()))
        .filter(l -> !fileName.equals(l.getLinkValue()))
        .toList();
    if(newLinks.isEmpty()) {
      merge.setAllLinks(TaskMapper.LINK_TYPE_ATTACHMENT, Collections.emptyList(), null);
    } else {
      merge.setAllLinks(
          TaskMapper.LINK_TYPE_ATTACHMENT, 
          newLinks, 
          newlink -> (builder) -> builder
          .linkType(TaskMapper.LINK_TYPE_ATTACHMENT)
          .linkBody(newlink.getLinkBody())
          .linkValue(newlink.getLinkValue())
          .build());
    }
    merge.build();
  }
  ...
  public ModifyOneMission start(GrimStructuredTenant config, ModifyOneMission builder) {
    builder.missionId(taskId).modifyMission(merge -> deleteTaskAttachment(name, merge));
    return builder
        .commitAuthor(userId)
        .commitMessage("Deleting tasks attachment " + name + " by: " + DeleteOneTaskAttachment.class.getSimpleName());
  }

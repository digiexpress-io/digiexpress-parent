  ...
  public QueryBuilder query() {
    return new QueryBuilder() {
      ...
      public Uni<Optional<io.resys.thena.api.entities.git.Branch>> getBranch() {
        final var branchToFind = branchName.orElse(getHeadName());
        ...
        return Uni.createFrom().item(() -> {
          ...
          while(iterator.hasNext()) {
            ...
            if(mapped.getBodyType() != AstBodyType.BRANCH) {
              continue;
            }
            
            if(branchToFind.equals(entry.getKey())) {
              return Optional.of(ImmutableBranch.builder().name(entry.getKey()).commit(entry.getValue().getRevision()).build());
            }
          }
    ...
    return new BranchQuery() {
      ...
      public Uni<List<Branch>> findAll() {
        return Uni.createFrom().item(() -> {
          ...
          final var result = new ArrayList<Branch>();
          final var cache = conn.getCacheManager().getCache(conn.getCacheName(), String.class, GitEntry.class);
          final var iterator = cache.iterator();
          while(iterator.hasNext()) {
            final var entry = iterator.next();            
            if(entry.getValue().getBodyType() != AstBodyType.BRANCH) {
              continue;
            }
            result.add(io.resys.hdes.client.api.ImmutableBranch.builder().name(entry.getKey()).commitId(entry.getValue().getRevision()).build());
          }

...
import io.resys.lp.batches.create_db.BatchJob_DROP_DB_Ledger.DropDbConfig;
...
public class BatchJob_DROP_DB_Ledger implements Executor<Tenant, DropDbConfig> {
  ...
  public ExecutorQuery<Tenant, DropDbConfig> before(ExecutorContext context) {
    return new ExecutorQuery<Tenant, DropDbConfig>() {
      ...
      public Multi<Tenant> findAll() {
        final var tenantId = ledgerClient.withTenant().getTenantId();
        return ledgerClient.tenants().queryTenants().id(tenantId).findAll();
      }
    };
  }
  ...
  public Uni<ExecutorEntity> accept(Tenant tenant, DropDbConfig config, ExecutorContext context) {
    if(tenant.getType() != StructureType.ledger) {
    ...
    return ledgerClient.tenants().queryTenants().id(tenant.getId()).deleteOne()

...
import io.resys.limaone.ast.ImmutableHeading;
import io.resys.limaone.ast.ImmutableImageTag;
...
public class MarkdownVisitor {
  ...
  @Value.Immutable
  public interface Markdown_AST {
    List<Article_AST.ImageTag> getImages();
    List<Article_AST.Heading> getHeadings();    
  }
  
  private final List<Article_AST.ImageTag> images = new ArrayList<>();
  private final List<Article_AST.Heading> headings = new ArrayList<>();
  
  public Markdown_AST visit(String content) {
    final MutableDataSet options = new MutableDataSet();
    final Parser parser = Parser.builder(options).build();
    final Node document = parser.parse(content);
    this.visitDocument(document);    
    return ImmutableMarkdown_AST.builder()
        .images(images)
        .headings(headings)
        .build();
  }

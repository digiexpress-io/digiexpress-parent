...
public class HttpClientImpl implements HttpClient {
  ...
  public AnyProxy anyProxy() {
    return new AnyProxy() {
      ...
      @Override
      public AnyProxy query(String query) {
        this.query = query;
        return this;
      }
      
      @Override
      public AnyProxy path(String path) {
        this.path = path;
        return this;
      }
      @Override
      public AnyProxy httpMethod(String httpMethod) {
        this.httpMethod = httpMethod;
        return this;
      }
      @Override
      public AnyProxy headers(Map<String, String> headers) {
        this.headers.putAll(headers);
        return this;
      }
      @Override
      public AnyProxy body(String body) {
        this.body = body;
        return this;
      }
      @Override
      public Uni<RawResponse> build() {
        Objects.requireNonNull(path, () -> "path must be defined");
        Objects.requireNonNull(httpMethod, () -> "httpMethod must be defined");
        
        final var method = HttpMethod.valueOf(httpMethod);
        ...
        return Uni.createFrom().item(() -> {
          ...
          try {
            final var response = restTemplate.exchange(path, method, new HttpEntity<>(body, reqHeaders), String.class);
            if(response.getStatusCode().is2xxSuccessful()) {
              log.debug("POST request successful, status: {}", response.getStatusCode());
              log.trace("POST response body: {}", response.getBody());
              return new RawResponseImpl(response);
            ...
            log.error("Server error during POST request to: {}, status: {}", path, e.getStatusCode(), e);
            throw new RuntimeException("Server error: " + e.getMessage(), e);
          } catch (HttpClientErrorException e) {
            log.error("Client error during POST request to: {}, status: {}", path, e.getStatusCode(), e);
            throw new RuntimeException("Client error: " + e.getMessage(), e);
          }

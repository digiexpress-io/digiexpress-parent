  ...
  public Uni<Model<PrintoutPage>> build() {
    return config.getPersistence().worldBuilder()
      ...
      .build(nextWorld -> {
        ...
        return nextWorld.mergeModel(props.getPageId(), body.getServiceId() + "_" + body.getLocaleId(), body, props.getAssetDescription(), props.getAssetLabels());
      });
  }
      ...
      for(final var resource : world.getPrintoutResources().values()) {
        ...
        if(isResourceInChanges && !isPageInResource) {
          final var newResource = ImmutablePrintoutResource.builder()
              ...
              .build();
          nextWorld.mergeModel(resource.getId(), newResource.getResourceName(), newResource, null, null);
        }

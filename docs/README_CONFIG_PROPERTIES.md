...
* `spring.ai.model.chat`, `spring.ai.model.embedding`: set each to `ollama` or `google-genai` when enabling Metis, `none` otherwise. They are independent, so chat on `google-genai` with embeddings on `ollama` works. Both `eveli-app` and `eveli-app-gcloud` ship `google-genai`; locally, `gcloud auth application-default login` provides the credentials.
...
Gemini on Vertex AI, used for whichever `spring.ai.model.*` key is `google-genai`. Authentication is Application Default Credentials (Workload Identity on GKE). See `mvn_setup/metis-parent/README.md#google-genai-vertex-ai` for the wiring, GCP setup and cost.

* `eveli.metis.google-genai.project-id`, `eveli.metis.google-genai.location`: GCP project billed for Vertex AI and its location. Use `eu`, the EU multi-region endpoint: it keeps processing in the EU and is the only EU location serving both default models. Required once `google-genai` is selected; boot fails with an explicit error otherwise.
* `eveli.metis.google-genai.chat.model`, `.chat.temperature`, `.chat.thinking-level`: defaults `gemini-3.1-flash-lite`, `0.2`, `MINIMAL`. Thinking tokens are billed as output and metadata generation does not need them.
* `eveli.metis.google-genai.embedding.model`, `.embedding.dimensions`: defaults `gemini-embedding-2` and `1024`. The width must equal `eveli.metis.search.indexing.embedding-dimension`; a reindex job checks what the model returns and fails before indexing any document on a mismatch.
...
* `eveli.metis.search.query.score-drop-off-ratio`: drop vector hits scoring below this fraction of the best match for the same query, default `0.90` (`bge-m3`); `0.95` for `gemini-embedding-2`. Not applied to a keyword-only list, so form-name hits stay.
* `eveli.metis.search.query.min-vector-score`: when even the best vector match is below this, the query has no semantic answer, default `0.30` (`bge-m3`); `0.60` for `gemini-embedding-2`.
* `eveli.metis.search.query.min-vector-score-without-keyword`: when primary full-text search is empty, a best vector score below this is treated as no answer, default `0.45` (`bge-m3`, where gibberish scores 0.30–0.45); `0.63` for `gemini-embedding-2`, where noise scores up to about 0.62. The three thresholds depend on the embedding model; see `mvn_setup/metis-parent/README.md#score-thresholds`. Does not apply when the query matched a title or other `websearch_to_tsquery` hit. Does not fall through to the trigram keyword fallback.
...
* Portal `GET /portal/site/search` answers `fallback: true` while the latest reindex job is `RUNNING`, `CANCELLING`, `FAILED`, or `CANCELLED`, or `COMPLETED` with another embedding model, so visitors keep keyword search until a job with the current model `COMPLETED`. `GET /worker/rest/api/metis/search/status` shows the failed or cancelled job so an operator can re-run reindex.

Indexing tuning:

* `eveli.metis.search.indexing.embedding-dimension`: documents the expected width of `metis_search_index.embedding`, `1024` for `bge-m3` and for `gemini-embedding-2` at its default setting. It does **not** alter the schema; the column is added as `VECTOR(1024)` when search is first enabled. A mismatch with the column or with what the model returns fails the reindex job. Changing the width is a schema concern.

...
interface HeadCell {
  id: keyof DeClient.DialobFormRevisionDocument;
}
...
const DescriptorTableHeader: React.FC<{
  content: DeClient.TablePagination<DeClient.DialobFormRevisionDocument>,
  setContent: React.Dispatch<React.SetStateAction<DeClient.TablePagination<DeClient.DialobFormRevisionDocument>>>
}> = ({ content, setContent }) => {

  const { order, orderBy } = content;

  const createSortHandler = (property: keyof DeClient.DialobFormRevisionDocument) =>
      ...
      {headCells.map((headCell) => (
        <TableCell key={headCell.id} align='left' padding='normal' sortDirection={orderBy === headCell.id ? order : false}>
          <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : 'asc'} onClick={createSortHandler(headCell.id)}>
            <FormattedMessage id={`dialobTable.header.${headCell.id}`} />
